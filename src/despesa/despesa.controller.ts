...
export class DespesaController {
  ...
  async create(@Body() createDespesaDto: CreateDespesaDto, @Req() req) {
    ...
    if (!painel || usuarioId !== painel.usuarioId) {
      console.log('Usuário não é o criador, verificando se é convidado...');
      ...
      if (!usuarioNoPainel) {
        console.log('Usuário não tem permissão para adicionar despesa.');
  ...
  async obterResumoDespesas(
    ...
    return this.despesaService.obterResumoDespesas(+painelId, periodo);
  }
}
